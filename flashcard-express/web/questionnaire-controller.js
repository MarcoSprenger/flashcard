...
exports.create = (req, res) => {
    ...
    questionnaire.save((err, questionnaireCreated) => {
        if (err) {
            ...
            logger.debug(
                `Successfully created questionnaire with id "${questionnaire.id}"`
            );
...
exports.update = (req, res) => {
    Questionnaire.findById(req.params.id, (err, questionnaire) => {
        if (err) {
            logger.error(
                `Could not update questionnaire with id "${req.params.id}"`
            );
            return res.status(404).send();
        }
        questionnaire.title = req.body.title;
        ...
        questionnaire.save((err) => {
            ...
            logger.debug(
                `Successfully updated questionnaire with id "${questionnaire.id}"`
            );
            res.status(200).json(questionnaire);
        });
    });
...
exports.delete = (req, res) => {
    Questionnaire.deleteOne({ id: req.params.id }, (err) => {
        if (err) {
            logger.error(
                `Could not delete questionnaire with id "${req.params.id}"`
            );
            return res.status(404).send();
        }
        logger.debug(
            `Successfully deleted questionnaire with id "${req.params.id}"`
        );
        res.status(200).send();
    });
};
